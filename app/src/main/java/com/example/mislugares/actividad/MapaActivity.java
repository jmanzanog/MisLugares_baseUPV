...
import com.example.mislugares.pojo.GeoPunto;
...
public class MapaActivity extends FragmentActivity
    ...
    @Override public void onMapReady(GoogleMap googleMap) {
        ...
        if (SelectorFragment.adaptador2.getItemCount() > 0) {
            GeoPunto p = SelectorFragment.adaptador2.getItem(0).getPosicion();
            mapa.moveCamera(CameraUpdateFactory.newLatLngZoom(
                    new LatLng(p.getLatitud(), p.getLongitud()), 12));
        }
        for (int n=0; n<SelectorFragment.adaptador2.getItemCount(); n++) {
            Lugar lugar = SelectorFragment.adaptador2.getItem(n);
    ...
    public void onInfoWindowClick(Marker marker) {
        for (int id=0; id<SelectorFragment.adaptador2.getItemCount(); id++){
            if (SelectorFragment.adaptador2.getItem(id).getNombre()
